...
import { BackpackItem, BodyPartStatus, BodyStatus, CalendarEvent, Comment, GameSave, GameTime, LocationID, Message, Product, SummaryEntry, Tweet } from '../types';
...
interface AppProps {
    ...
    todaySummaries?: SummaryEntry[]; // For CalendarApp - 全部小总结
                ...
                {chatHistory.length === 0 ? (
                    ...
                    chatHistory.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <div className={`max-w-[70%] px-3 py-2 rounded-lg text-sm leading-relaxed shadow-sm ${msg.sender === 'user'
                        ...
                        return (
                            <div key={idx} className="space-y-2">
                                <button
                                    ...
                                    className={`w-full p-4 rounded-2xl flex items-center justify-between group transition-all border ${fac.comingSoon
                                            ? 'bg-gray-100 opacity-60 cursor-not-allowed'
                                            : 'bg-gray-50 hover:bg-blue-50 border-transparent hover:border-blue-200'
                                        }`}
                                >
                                    <div className="flex items-center gap-4 flex-1">
                                        <div className={`w-12 h-12 rounded-xl bg-white shadow-sm flex items-center justify-center group-hover:scale-110 transition-transform ${fac.comingSoon ? 'text-gray-400' : fac.isWork ? 'text-green-500' : 'text-blue-500'
                                            }`}>
                                            <fac.icon size={24} />
                                        </div>
                                        <div className="flex-1">
                                            ...
                                            {fac.price && (
                                                <div className={`text-sm font-bold mt-1 ${fac.isWork ? 'text-green-600' : 'text-orange-600'
                                                    }`}>
                                                    {fac.isWork
                                                        ? `+¥${fac.price}/小时`
                                                        : fac.priceForTwo
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            {selectedFacility.movieTypes.map((type) => (
                                                <button
                                                    ...
                                                    className={`py-2 px-4 rounded-xl font-bold text-sm transition-all ${selectedMovieType === type
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <div className="grid grid-cols-3 gap-2">
                                                <button
                                                    ...
                                                    className={`py-2 px-3 rounded-xl font-bold text-sm transition-all ${selectedSeatType === 'normal'
                                                ...
                                                <button
                                                    ...
                                                    className={`py-2 px-3 rounded-xl font-bold text-sm transition-all ${selectedSeatType === 'vip'
                                                            ? 'bg-purple-500 text-white shadow-lg'
                                                            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                                                        }`}
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className={`py-2 px-3 rounded-xl font-bold text-sm transition-all ${selectedSeatType === 'couple'
...
export const CalendarApp: React.FC<AppProps> = ({
    ...
    const formatDiaryDateTime = (time: GameTime) => {
    ...
    {
        return (
            <div className="flex flex-col h-full bg-[#fffafc]">
                <Header title="温婉的日记" onClose={onClose} onBack={() => setView('CALENDAR')} rightIcon={<Sparkles size={18} className="text-pink-500 mr-2" />} />
                <div className="flex-1 overflow-y-auto px-4 py-4">
                    {todaySummaries.length === 0 ? (
                        <div className="flex h-full min-h-[280px] flex-col items-center justify-center rounded-[28px] border border-pink-100 bg-white/80 px-6 text-center shadow-sm">
                            <Sparkles size={28} className="mb-3 text-pink-300" />
                            <div className="text-sm font-semibold text-gray-700">还没有写下新的日记</div>
                            <div className="mt-2 text-xs leading-relaxed text-gray-400">再和温婉多聊几轮，等她慢慢把今天的心事记下来。</div>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {[...todaySummaries].reverse().map((entry, index) => (
                                <div key={`${entry.gameTime.year}-${entry.gameTime.month}-${entry.gameTime.day}-${entry.gameTime.hour}-${entry.gameTime.minute}-${index}`} className="overflow-hidden rounded-[28px] border border-pink-100 bg-gradient-to-br from-white via-pink-50/70 to-rose-50/80 shadow-[0_12px_28px_rgba(244,114,182,0.10)]">
                                    <div className="border-b border-pink-100/80 px-5 py-3">
                                        <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-pink-400">Diary Entry</div>
                                        <div className="mt-1 text-xs font-medium text-gray-500">{formatDiaryDateTime(entry.gameTime)}</div>
                                    </div>
                                    <div className="px-5 py-4">
                                        <p className="text-sm leading-7 text-gray-700 whitespace-pre-line">{entry.content}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                ...
                {todaySummary && (
                    <button
                        ...
                        className="w-full rounded-2xl border border-pink-200 bg-gradient-to-r from-pink-50 via-rose-50 to-orange-50 p-4 text-left transition-all active:scale-[0.98] hover:border-pink-300"
                    >
                        <div className="mb-2 flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2">
                                <Sparkles size={16} className="text-pink-500" />
                                <h3 className="text-sm font-bold text-gray-900">今日总结</h3>
                            </div>
                        ...
                        <p className="text-xs leading-relaxed text-gray-700 whitespace-pre-line">{todaySummary}</p>
                        <div className="mt-3 text-[11px] text-gray-400">共 {todaySummaries.length} 条小总结，点击进入「温婉的日记」</div>
                    </button>
                )}
            ...
            <div className="flex-1 overflow-y-auto p-4">
                <div className="grid grid-cols-2 gap-3">
                    {saves.map((save, index) => (
                        <div
                            key={index}
                            className={`border-2 rounded-xl p-3 transition-all ${save
            ...
            <div className="mt-2 h-1.5 w-full bg-pink-100 rounded-full overflow-hidden">
                ...
                */}
                <div className="h-full bg-pink-400 transition-all duration-500" style={{
                    width: `${level === 0 ? (part.usageCount / 50) * 100 :
